...
(define (ifarith-tiny? e)
  (match e
    ...
    [(? symbol? x) #t]
    ;; applications of primitives, our language has no lambdas
    [`(,(? bop? bop) ,(? ifarith-tiny? e0) ,(? ifarith-tiny? e1)) #t]
    [`(,(? uop? bop) ,(? ifarith-tiny? e0)) #t]
    ;; let* works in the usual way
    [`(let ([,(? symbol? x) ,(? ifarith-tiny? e)]) ,(? ifarith-tiny? e-body)) #t] 
    ;; print an arbitrary expression (must be a number at runtime)
    [`(print ,(? ifarith-tiny? e)) #t]
    ;; if argument is 0, false, otherwise true
    [`(if ,(? ifarith-tiny? e0) ,(? ifarith-tiny? e1) ,(? ifarith-tiny? e2)) #t]
    [_ #f]))
...
(define (ifarith->ifarith-tiny e)
  (match e
    ...
    [`(let* ([,(? symbol? x0) ,e0] ,rest-binding-pairs ...) ,e-body)
      `(let ([,(ifarith->ifarith-tiny x0) ,(ifarith->ifarith-tiny e0)]) ,(ifarith->ifarith-tiny `(let* (,@rest-binding-pairs) ,e-body)))]
    ;; print an arbitrary expression (must be a number at runtime) 
    [`(print ,_) e]
    ;; and/or, with short-circuiting semantics
...
; (ifarith->ifarith-tiny '(let* ([a 3] [b 1]) (+ a b)))
;(ifarith->ifarith-tiny '(let* ([a 3] [b 2]) (+ 3 4)))
